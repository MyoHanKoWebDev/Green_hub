...
const Productfilter = () => {
  ...
  useEffect(() => {
    axios.get("api/admin/projects").then((res) => setProjects(res.data.data));
  }, []);
  ...
  const dynamicFilters = [
    {
      ...
      options: projects.map((proj) => ({
        value: proj.id, // The ID to send to the backend
        label: proj.title, // The name to show in UI
      })),
    },
  ];
  ...
  }, [search, projectId, priceRange.min, priceRange.max, currentPage, sortBy, location.state]);
            ...
            <form className="mt-6 border-t border-gray-200">
              {dynamicFilters.map((s) => (
                <FilterSection
                  ...
                  onSelect={(id) => {
                    // Toggle logic: if clicking the same one, clear it (null)
                    const newId = projectId === id ? null : id;
                    setProjectId(newId);
                    setCurrentPage(1);
                  }}
                  isMobile
