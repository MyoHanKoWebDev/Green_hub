...
export const PriceFilter = ({isMobile=false ,onApply, initialValues}) => {
  ...
  // Auto-apply when user stops typing (Optional) or use the button
  const handleApply = (e) => {
  ...
  <Disclosure as="div" className={`border-b border-gray-200 py-6 ${isMobile ? "px-4" : ""}`}>
    <h3 className="-my-3 flow-root">
      <DisclosureButton className="group flex w-full items-center justify-between bg-white py-3 text-sm text-gray-400 hover:text-gray-500">
        ...
        <span className="ml-6 flex items-center"><PlusIcon className="size-5 group-data-open:hidden" /><MinusIcon className="size-5 group-not-data-open:hidden" /></span>
      </DisclosureButton>
    </h3>
    <DisclosurePanel transition className="pt-6 transition duration-300 ease-out data-closed:-translate-y-4 data-closed:opacity-0">
      <div className="space-y-4">
        ...
        <button onClick={handleApply} type="button" className="w-full bg-lime-500 hover:bg-lime-600 text-white text-xs font-bold py-2.5 rounded-md transition-all active:scale-95">Apply Price</button>
      </div>
    </DisclosurePanel>
  </Disclosure>
);
